...
//            .setScrollFactor(0);
        ...
        this.physics.add.collider(this.player.sprite, this.roboTriggerPoint, 
                                  function(){ eventsCenter.emit('robotTriggered'); }, null, this); // if player collides with roboTriggerPoint, perform robotEnter
        eventsCenter.once('robotTriggered', robotEnter, this);  

        // 2. check for overlap between player and robots:
        this.physics.add.overlap(this.player.sprite, this.robo1.sprite, 
                                 function(){ eventsCenter.emit('robotEncountered'); }, null, this); // if player overlaps with robo1 sprite, perform robotEncounter
        this.physics.add.overlap(this.player.sprite, this.robo2.sprite, 
                                 function(){ eventsCenter.emit('robotEncountered'); }, null, this); // if player overlaps with robo2 sprite, perform robotEncounter
        eventsCenter.once('robotEncountered', robotEncounter, this);  

        // 3 . check for collsion between player and trial end point (invisible):
        eventsCenter.once('ratingCompleteA',
            function (){
                this.physics.add.collider(this.player.sprite, this.trialEndPoint, 
                                          function(){ eventsCenter.emit('trialEndHit'); }, null, this);  // if player collides with trialEndPoint, perform trialEnd
...
var robotEncounter = function () {
    ...
    eventsCenter.once('ratingCompleteA', function () {
        // get rating time
...
var trialEnd = function () {
    ...
    saveTaskData(trial, this.registry.get(`trial${trial}`));        
